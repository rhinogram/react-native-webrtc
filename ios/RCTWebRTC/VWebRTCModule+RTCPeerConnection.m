//
//  VWebRTCModule+RTCPeerConnection.m
//
...
#import "VWebRTCModule+RTCDataChannel.h"
#import "VWebRTCModule+RTCPeerConnection.h"
#import "VWebRTCModule+VideoTrackAdapter.h"
...
- (void)setWebRTCModule:(id)vWebRTCModule
{
    objc_setAssociatedObject(self, @selector(vWebRTCModule), vWebRTCModule, OBJC_ASSOCIATION_ASSIGN);
}

@end

@implementation VWebRTCModule (RTCPeerConnection)
                            ...
                            objectID:(nonnull NSNumber *)objectID)
{
  ...
  peerConnection.vWebRTCModule = self;
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection didRemoveStream:(RTCMediaStream *)stream {
  // XXX Find the stream by comparing the 'streamId' values. It turns out that VWebRTC (as of M69) creates new wrapper
...
- (void)peerConnection:(RTCPeerConnection*)peerConnection didOpenDataChannel:(RTCDataChannel*)dataChannel {
  // XXX RTP data channels are not defined by the VWebRTC standard, have been
  ...
  // VWebRTCModule implements the category RTCDataChannel i.e. the protocol
