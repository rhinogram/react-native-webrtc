...
export default class RTCDataChannel extends EventTarget(DATA_CHANNEL_EVENTS) {
  ...
  send(data: string | ArrayBuffer | ArrayBufferView) {
    if (typeof data === 'string') {
      VWebRTCModule.dataChannelSend(this._peerConnectionId, this.id, data, 'text');
      return;
    }
    ...
    VWebRTCModule.dataChannelSend(this._peerConnectionId, this.id, base64.fromByteArray(data), 'binary');
  }
  ...
  close() {
    ...
    VWebRTCModule.dataChannelClose(this._peerConnectionId, this.id);
  }
